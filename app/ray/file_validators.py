...
def validate_json(file_path: str) -> Tuple[bool, str]:
    """Validates JSON file content using streaming parser (memory-efficient)."""
    try:
        with open(file_path, "rb") as f:
            # ijson.parse() yields events incrementally
            # iterating through all events validates the entire structure
            for __ in ijson.parse(f):  # double underscore
                pass
        return True, ""
