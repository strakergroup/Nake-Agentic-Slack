...
def probe_bitrate(url: str, token: str) -> int:
    """Probe a media file to determine its actual audio bitrate.

    Uses ffmpeg to quickly extract bitrate information without downloading
    the entire file. Returns the bitrate in kbps or 0 if detection fails.
    """
    try:
        # Run ffprobe directly to get stream info with minimal download
        import subprocess
        import json
        ...
        if "streams" in data and data["streams"] and "bit_rate" in data["streams"][0]:
...
def estimate_audio_size(duration_ms: int, url: str = None, token: str = None) -> int:
    """Ultra-fast estimation of processed audio size using bitrate calculation.

    For MP3 at 32kbps (the optimal format for Whisper-1):
    - Bitrate is 32,000 bits per second = 4,000 bytes per second
    - Adding 1% overhead for MP3 headers and metadata

    If url and token are provided, attempts to probe the file for its actual bitrate.

    This provides immediate results with excellent accuracy for speech files.
    """
    try:
        # Default bitrate for Whisper-1 optimal processing
        BITRATE_KBPS = 32  # 32 kilobits per second

        # If URL and token provided, try to probe for actual bitrate
        detected_bitrate = 0
        if url and token:
            detected_bitrate = probe_bitrate(url, token)

        if detected_bitrate > 0:
            # Use detected bitrate, but cap it at a reasonable value to avoid overestimation
            BITRATE_KBPS = min(detected_bitrate, 256)

        # Calculate size based on bitrate - with overhead for headers/metadata
        bytes_per_second = (BITRATE_KBPS * 1000 / 8) * 1.01
        estimated_size = int((duration_ms / 1000) * bytes_per_second)

        return estimated_size

    except Exception:
        # Use basic bitrate calculation as fallback
        fallback_size = int((duration_ms / 1000) * (32000 / 8) * 1.01)
