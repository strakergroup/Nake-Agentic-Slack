...
def all_text(value):
    ...
    if isinstance(value, dict):
        return [
            ...
            if k not in {"action_id", "block_id", "value", "type", "style"}
            for t in all_text(v)
        ]
...
def test_status_payload_has_title_only_when_given():
    facts = make_facts()
    assert slack_ui.status_payload(facts, "processing") == {
        "channel_id": "D1",
        "thread_ts": "111.222",
        "status": "processing",
    }
...
def test_stream_start_names_the_recipient_outside_a_dm():
    ...
    assert (
        dm["task_display_mode"] == "plan"
        and "recipient_user_id" not in dm
        and "chunks" not in dm
    )
    channel = slack_ui.stream_start_payload(
        make_facts(surface="mention", channel_id="C1"), "Hello"
    )
    assert (
        channel["recipient_user_id"] == "U_MIKA"
        and channel["recipient_team_id"] == "T1"
    )
...
def test_task_chunks_map_card_states():
    ...
    assert [c["task"]["status"] for c in chunks] == [
        "complete",
        "pending",
        "in_progress",
    ]
    assert (
        chunks[0]["task"]["output"]["elements"][0]["elements"][0]["text"] == "2 found"
    )
...
def test_amount_only_shown_when_allowed():
    ...
    shown = slack_ui.approval_blocks(
        approval(show_amount=True, amount_text="USD 12.00")
    )
...
def test_handoff_reuses_the_existing_openers_and_their_payload_shape():
    ...
    assert json.loads(button["value"]) == {
        "files": [{"id": "F1", "title": "town-hall.mp4"}],
        "channel_id": "D1",
        "thread_ts": "111.222",
    }
    assert (
        slack_ui.handoff_blocks("document_translation", files, "D1", None)[1][
            "elements"
        ][0]["action_id"]
        == "document_mt_job"
    )
...
def test_home_blocks_hide_channel_list_from_non_admins():
    ...
    admin = slack_ui.home_blocks(
        "off", muted, [{"label": "#launch-global, on", "channel_id": "C1"}]
    )
...
def test_all_builder_text_follows_the_voice_rules_and_uses_no_emoji():
    samples = [
        ...
        *[
            slack_ui.handoff_blocks(form, [], "D1", None)
            for form in slack_ui.FORM_ACTION_IDS
        ],
    ]
    bad = [
        f"{v.rule}: {t}"
        for blocks in samples
        for t in all_text(blocks)
        for v in check_copy(t)
    ]
    assert bad == []
