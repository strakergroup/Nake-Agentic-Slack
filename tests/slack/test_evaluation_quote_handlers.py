...
@pytest.mark.asyncio
async def test_ai_quote_adjust_maps_document_mt_action_to_quote_kind():
    with (
        patch(
            "app.slack.handlers.evaluate.open_loading_modal",
        ...
        await AiQuoteAdjustAction(
            ...
            action={
                "value": "quote-1",
                "action_id": "document_mt_quote_adjust",
            },
            context={},
        )

    assert mock_populate.await_args.kwargs["quote_kind"] == "document_mt"
    assert mock_populate.await_args.kwargs["quote_id"] == "quote-1"


@pytest.mark.asyncio
async def test_ai_quote_adjust_submit_document_mt_accepts_document_mt_quote():
    ack = AsyncMock()
    client = AsyncMock()
    context = {"channel_id": "C1"}
    view = {
        ...
        "private_metadata": (
            '{"quote_id":"quote-1","quote_kind":"document_mt",'
            '"channel_id":"C1","message_ts":"111.222"}'
        ),
    }
    ...
    mock_accept_document_mt.assert_awaited_once_with(
        client=client,
        body=body,
