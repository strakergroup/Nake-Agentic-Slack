...
When `verify:slack:evaluate:complete` arrives, SRT validates that refreshed totals include the exact QE charges for the **selected** file/language pairs only, then **replaces the original HT/QE quote message** with the final-cost status only (`Final cost after AI quality evaluation: USD $X`, estimated completion date, and confirmation that the AI translation was submitted to specialist linguists). No separate follow-up message is posted. The AI Translation quote message stays in place above with the same per-file/per-language layout (deselected Adjust Request pairs shown as **Cancelled**) and an AI-complete status line.
