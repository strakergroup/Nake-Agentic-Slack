...
| `app/agent/core/tools.py` | `ToolRegistry`: specs for the ten tools, each marked `lookup` or `gated`, with strict JSON schemas. Handlers are injected. |
...
| `submit_document_translation` | **gated** | People who cannot see quotes: submit attached files, only after the confirming click (Decision D2). |
| `post_translation_publicly` | **gated** | Post a translation into a channel thread on the person's behalf, only after a click. |
| `offer_form` | lookup | Show a button that opens one of their existing forms: media, quality evaluation, human translation, new job, channel settings. |
...
- [ ] **Step 1: Tests.** The registry lists exactly the ten tools in the table above; exactly two are `gated`; every schema has `"additionalProperties": False` and a `required` list (needed for strict tool use); `as_anthropic_tools()` emits `{"name","description","input_schema","strict": True}`; registering a handler for an unknown tool raises; `offer_form` only accepts `form` values from the closed set `media, quality_evaluation, human_translation, new_job, channel_settings`; `translate_text` requires `target_language` and one of `text` or `use_current_thread`; descriptions pass `check_copy`.
...
| `submit_document_translation` | `enqueue_document_mt_submission(..., quote_id=None)` (`dispatch.py:79`) | Gated. Exactly the call their form makes for non-admins at `handlers/document_mt.py:301`. Never exposed for people who can see quotes. |
...
Explicitly never bound: `document_machine_translate` (`listener_actions.py:1232`), `handle_document_mt_submit` (`handlers/document_mt.py:88`), `accept_document_mt_quote`, `RayService.new_job`, `cancel_job`. A test in `test_straker_tools.py` parses the adapter's source and fails if any of those names appears.
...
- [ ] **Step 5: POC corrections** from the code mapping: scenario 4 becomes a button ("Subtitles run through the media form." then **Open media form**); if Decision D1 is yes, "Credits" becomes the `USD 12.00` format; a non-admin variant of the document scenario shows the no-price confirmation (Decision D2). Re-run the POC's 33 tests, republish to the same link, re-verify. This is also when the unverified mouse-click question on the live POC gets settled.

---

## Task 14: Documents and the hand-over pack

- [ ] **Step 1: `docs/arbitr-agent.md`**: the two layers, the seam, the flags, the ten tools and their bindings, exactly what data reaches Claude and what never does, failure behaviour, the agent's Redis keys, what the audit record contains.
- [ ] **Step 2: `docs/arbitr-agent-runbook.md`** for Wade's team: regenerate the lock file; set the variables; create a dev Slack app from `manifest.agent.yml`; then a numbered checklist, each with the expected result: flag off behaves as today; flag on, "where's my job" answers from RAY; admin document request produces their quote message; non-admin gets the confirmation and one click submits once; a second click does nothing; Stop works; IBM-flagged workspace shows no connect or top-up wording; suggestions stay silent until enabled; their full test suite still passes. It ends with "open questions for Wade": the `usage_type` value, `is_ibm_customer_enterprise` versus `is_ibm_enterprise`, whether the translated-document event can carry `quote_id`, session storage moving from Redis to a table, the web process sharing with the background worker, and scope approval for `assistant:write`.
...
- [ ] **Step 4: Patch bundle.** In the clone: `git format-patch origin/master..arbitr-agent -o ../handover/patches` and `git bundle create ../handover/arbitr-agent.bundle origin/master..arbitr-agent`. Nothing is pushed.
